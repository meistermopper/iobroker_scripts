...
async function initDP() {
  ...
  const states = [
    { id: IDS.userMaeht, val: false, type: "boolean", name: "R2Mäh2 mäh-Status" },
    {
      ...
      val: [0, 0, 0, 0, 0, 0, 0],
      type: "array",
      name: "Historie der letzten 7 Tage",
    },
...
function startStuckTimer() {
  ...
  stuckTimer = setTimeout(async () => {
    const msg =
      "Achtung: Erzwo mäh zwo mäht seit über 120 Minuten. Er ist vermutlich irgendwo liegen geblieben.";
    await sendGlobalNotify(
      `⚠️ ${msg}`,
      "R2Maeh2",
      2,
      compareTime("08:00", "20:00", "between") ? 45 : null,
    ); // Sprachausgabe nur tagsüber
...
on({ id: IDS.power, change: "ne" }, async (obj) => {
  if (!isSaison()) return;
  if (!obj.state) return;
  ...
  if (
    compareTime("10:00", "18:01", "between") &&
    watt < THRESHOLD_IDLE &&
    oldWatt >= THRESHOLD_IDLE &&
    !maeht
  ) {
    maeht = true;
