...
function getFensterStatus(user) {
    ...
    const msg = offene.length === 0 ? "✅ <b>Alle Fenster/Türen sind zu.</b>" : `⚠️ <b>Offen:</b>\n\n- ${offene.join('\n- ')}`;
    smartNotify(user, msg, prio);
}
...
function menuSchalter(user) {
    showMenu(user, '<b>💡 Schaltungen</b>\nWas soll geschaltet werden?', [
        [{ text: '🍖 Drehspieß AN', callback_data: 'sp_on' }, { text: '⚪ OFF', callback_data: 'sp_off' }],
        [{ text: '🌳 Terrasse AN', callback_data: 'tr_on' }, { text: '⚪ OFF', callback_data: 'tr_off' }],
        [{ text: '🔌 Steckleiste AN', callback_data: 'st_on' }, { text: '⚪ OFF', callback_data: 'st_off' }],
...
on({ id: 'telegram.0.communicate.request', change: 'any' }, async (obj) => {
    ...
    const cmd = val.substring(val.indexOf(']') + 1).trim().toLowerCase();
    ...
    switch (cmd) {
        ...
        case 'termine':
            ...
            if (!tState || !tState.val) { smartNotify(user, "Keine Termine."); }
            else {
                let heute = new Date().setHours(0,0,0,0);
                let tMsg = '';
                tState.val.forEach(t => {
                    let diff = Math.floor((new Date(t._date).setHours(0,0,0,0) - heute) / 86400000);
                    if (diff >= 0 && diff <= 2) tMsg += `📅 <b>${diff === 0 ? 'Heute: ' : (diff === 1 ? 'Morgen: ' : '')}</b> ${t.event}\n`;
                });
                smartNotify(user, tMsg === '' ? "Keine Termine." : `<b>Termine:</b>\n${tMsg}`);
            }
            break;
        case 'astro':
            const sr = getAstroDate('sunrise', undefined, 0).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
            const ss = getAstroDate('sunset', undefined, 0).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
