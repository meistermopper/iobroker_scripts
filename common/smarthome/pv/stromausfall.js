...
async function initGridMonitor() {
    if (!existsState(dpPersistPath)) {
        await createStateAsync(dpPersistPath, 0, { name: "Zeitpunkt Netzausfall", type: "number", role: "value.datetime" });
    }
}

function gridNotify(msg, priority = 5) {
    sendTo("telegram", "send", { text: `<pre>🌐 NETZ-MONITOR\n\n${msg}</pre>`, parse_mode: "HTML" });
    exec(`curl "https://mygotify.meistermopper.de/message?token=${gotifyToken}" -F "title=Netzstatus Haus" -F "message=${msg}" -F "priority=${priority}"`);
}
...
on({ id: dpGridAlarm, change: "ne" }, async (obj) => {
    ...
    if (status === 2) {
        ...
        gridNotify(`🔌 ALARM: Stromnetz ausgefallen!\nInfo: Sprachausgabe deaktiviert (Geräte stromlos).`, 8);
