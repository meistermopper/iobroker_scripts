...
schedule({ astro: "goldenHour", shift: 0 }, async () => {
  ...
  if (
    ...
    getState(
      "modbus.0.inputRegisters.100.843_Battery_State_of_Charge_(System)",
    ) &&
    getState("modbus.0.inputRegisters.100.843_Battery_State_of_Charge_(System)")
      .val < 84 &&
    getState(
      "modbus.0.inputRegisters.100.2901_ESS_Minimum_SoC_(unless_grid_fails)",
    ) &&
    getState(
      "modbus.0.inputRegisters.100.2901_ESS_Minimum_SoC_(unless_grid_fails)",
    ).val < 40
...
on(
  {
    id: "modbus.0.inputRegisters.100.843_Battery_State_of_Charge_(System)",
    change: "ne",
  },
  async (obj) => {
    if (obj && obj.state && obj.oldState) {
      ...
      if (value === 96 && oldValue < 96 && !message) {
        ...
        if (getState("0_userdata.0.gotifytoken.iobroker")) {
          exec(
            'curl "https://mygotify.meistermopper.de/message?token=' +
              ...
              '" -F "title=ioBroker:\n" -F "message=👌 Bingo! Die Hausbatterie ist proppevoll." -F "priority=1"',
            (error, stdout, stderr) => {
              if (error) {
        ...
        if (
          ...
          (new Date().getMonth() + 1 > 9 || new Date().getMonth() + 1 < 4)
        ) {
          if (
            getState(
              "modbus.0.inputRegisters.100.2901_ESS_Minimum_SoC_(unless_grid_fails)",
            ).val !== 30 &&
            getState("zigbee.0.00158d0005435fe1.opened") &&
            getState("zigbee.0.00158d0005435fe1.opened").val
          ) {
            setState(
              "modbus.0.holdingRegisters.100.2901_ESS_Minimum_SoC_(unless_grid_fails)",
              30,
            ...
            if (getState("0_userdata.0.gotifytoken.iobroker")) {
              exec(
                'curl "https://mygotify.meistermopper.de/message?token=' +
                  getState("0_userdata.0.gotifytoken.iobroker").val +
                  '" -F "title=ioBroker:\n" -F "message=Winterhalbjahr: 🔋Min SoC wurde auf 30% festgelegt." -F "priority=1"',
                (error, stdout, stderr) => {
                  if (error) {
                    console.error(`exec error: ${error}`);
                    return;
