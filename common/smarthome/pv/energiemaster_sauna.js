// =============================================================================
// UNIVERSAL MASTER v2.4 (PRECISION YIELD & ASTRO-REPORT)
// =============================================================================
...
const IDS = {
    ...
    pvYieldToday: 'solax.0.data.yieldtoday', // NEU: Hardware-Datenpunkt
...
const SAUNA_POWER = 7500, WALLBOX_WATT_FIX = 3960, RADIO_SENDER = 'smoothjazz';
const GOTIFY_TOKEN_ID = '0_userdata.0.gotifytoken.iobroker', GOTIFY_SERVER = 'mygotify.meistermopper.de';
...
let tSaunaStart = null, tSaunaReset = null, tRadioBad = null, tRadioSauna = null, tTuerWarn = null;
...
async function initSystem() {
    ...
    for (const s of states) { await createStateAsync(s.id, s.def, { type: typeof s.def, unit: s.unit }); }
    ...
    console.log("[Master] v2.4 (Precision) bereit.");
}
initSystem();

// --- 3. HILFSFUNKTIONEN ---
function formatMins(m) { if (isNaN(m) || m <= 0 || !isFinite(m)) return "n. n."; return `${Math.floor(m / 60)}:${(m % 60) < 10 ? '0' + Math.floor(m % 60) : Math.floor(m % 60)}`; }
function announce(t) { SAYIT_INSTANCES.forEach(i => { sendTo(i, "say", { text: t, volume: 70 }); }); }
function notify(title, msg, prio = 5) {
    sendTo('telegram', 'send', { text: msg });
    const token = getState(GOTIFY_TOKEN_ID).val;
    if (token) httpPost(`https://${GOTIFY_SERVER}/message?token=${token}`, { title: title, message: msg, priority: prio });
}
function getBereinigteLast() {
    const hV = (Number(getState(PATH_PV + 'Hausverbrauch').val) || 0);
    let sum = 0; IDS.verbraucherWatt.forEach(id => { sum += (Number(getState(id).val) || 0); });
    if (getState(IDS.wallboxStatus).val === 'Charging') sum += WALLBOX_WATT_FIX;
    return hV - sum;
}
...
function runMasterUpdate() {
    ...
    const lKWh = (sMax * soc) / 100;
    let fUm = "n. n.", rMin = 0;
    if (batP > 50) {
        const rSec = ((sMax - lKWh) / (batP / 1000)) * 3600;
        rMin = rSec / 60;
        let t = new Date(); t.setSeconds(t.getSeconds() + rSec); fUm = formatDate(t, "hh:mm");
    }

    const curAut = hausV > 0 ? Math.round(Math.min(100, (1 - ((netP > 0 ? netP : 0) / hausV)) * 100)) : 0;
    ...
    if (bLast > SAUNA_POWER) {
        if (!sL && !tSaunaStart) {
            tSaunaStart = setTimeout(() => { if (getBereinigteLast() > SAUNA_POWER) { startSauna(getBereinigteLast()); } tSaunaStart = null; }, 30000);
        } else if (sL) { const aSoc = getState(IDS.batSoc).val; if (aSoc > getState(IDS.minSocRead).val) setState(IDS.minSocSet, aSoc); }
    } else if (bLast < 1000) {
        if (sL && !tempMeldungGesendet && !tSaunaReset) { tempMeldungGesendet = true; notify('Sauna', '🌡️ Zieltemperatur erreicht!'); announce("Die Sauna ist jetzt heiß."); }
        stopSauna();
    }
}
...
function startSauna(watt) {
    ...
    notify('Sauna', '🧖‍♂️ Sauna eingeschaltet.');
    tRadioBad = setTimeout(() => { setState(IDS.heos.badSender, RADIO_SENDER); setState(IDS.heos.badStatus, true); }, 900000);
}
...
// --- 5. ASTRO-REPORT (Nutzt Hardware-Datenpunkt) ---
schedule({astro: "sunset"}, () => {
    const yieldKWh = getState(IDS.pvYieldToday).val || 0; // Direkter Hardware-Wert
    let dAut = (tVerbrauchWh > 0) ? Math.round((1 - (tNetzWh / tVerbrauchWh)) * 100) : 0;
    
    let msg = `☀️ *PV-Tagesbericht*\n\n`;
    msg += `PV Erzeugung: ${yieldKWh.toFixed(2)} kWh\n`;
    msg += `Hausverbrauch: ${(tVerbrauchWh/1000).toFixed(2)} kWh\n`;
    msg += `Echte Autarkie: ${dAut}%\n`;
    msg += `Batterie Ladung: ${(tLadungWh/1000).toFixed(2)} kWh`;
    
    notify('☀️ PV-Tagesbericht', msg);
});

// --- 6. TRIGGER ---
on({id: IDS.pvPower, change: "ne"}, (obj) => { pvP = Number(obj.state.val) || 0; runMasterUpdate(); });
on({id: IDS.netPower, change: "ne"}, (obj) => { netP = Number(obj.state.val) || 0; runMasterUpdate(); });
on({id: IDS.batPower, change: "ne"}, (obj) => { batP = Number(obj.state.val) || 0; runMasterUpdate(); });
on({id: IDS.batSoc, change: "ne"}, (obj) => { soc = Number(obj.state.val) || 0; });
on({id: IDS.saunaTuer, change: 'ne'}, (obj) => {
    if (obj.state.val === true && getState(IDS.saunaLogik).val) {
        tTuerWarn = setTimeout(() => { if (getState(IDS.saunaTuer).val && getState(IDS.saunaLogik).val) { announce('Achtung! Die Saunatür ist offen!'); notify('Sauna Alarm', 'Tür offen!'); } }, 60000);
    } else if (tTuerWarn) { clearTimeout(tTuerWarn); tTuerWarn = null; }
});

setInterval(() => {
    // Synchronisiere Hardware-Yield mit internem Wh-Datenpunkt
    const yieldWh = (getState(IDS.pvYieldToday).val || 0) * 1000;
...
schedule("0 0 * * *", () => { tVerbrauchWh = 0; tLadungWh = 0; tNetzWh = 0; });