...
on({ id: 'bluelink.0.KNAFD81A7S6058382.vehicleStatusRaw.Green.PowerConsumption.Prediction.Climate', change: 'ne' }, async (obj) => {
...
async function starteHeizung() {
    ...
    const keineArbeit = getState(IDS.holiday).val || getState(IDS.vacation1).val || getState(IDS.vacation2).val;
    ...
    if (heizenAktiv && amStandort && !keineArbeit) {
        ...
        if (aktuellTemp <= TEMP_DEFROST) {
            ...
            sendTo('telegram', 'send', { text: `+++ ♨️ ❄️ Kia: Entfrosten gestartet bei ${aktuellTemp} °C +++` });
        } else if (aktuellTemp < TEMP_MAX_HEAT) {
            setState(IDS.climaStart, true);
            sendTo('telegram', 'send', { text: `+++ ♨️ Kia: Heizen gestartet bei ${aktuellTemp} °C +++` });
        }
    }
}
...
schedule("37 9 * * 1", () => { starteHeizung(); }); 
//schedule("0 8 * * 2", () => { starteHeizung(); });
schedule("0 8 * * 3", () => { starteHeizung(); });
