...
const IDS = {
  ...
  // Externe Datenpunkte
  chargingActive:
    "ocpp.0.http://192_168_178_80:9220/EVB-P21312507.1.transactionActive",
...
on({ id: /^vis\..*\.control\.data$/, change: "any" }, async (obj) => {
  ...
  if (
    viewPath === "projektx/960_Auto" ||
    viewPath === "projektx_sp/963_Standort_eNiro"
  ) {
    const now = Date.now();
    ...
    if (now > viewTriggerLock) {
      console.warn(
        `[Kia] Gezielte Fahrzeug-Abfrage durch View-Aufruf: ${viewPath}`,
      );
      ...
      console.log(
        `[Kia] View ${viewPath} aufgerufen, aber Abfrage noch im Cooldown.`,
      );
    }
  }
});
...
on(
  {
    id: [IDS.refreshCar, IDS.refreshSrv, `${VIN}.control.force_refresh`],
    change: "any",
  },
  async (obj) => {
...
async function processLocationUpdate() {
  ...
  if (
    lat.toFixed(4) === lastLat.toFixed(4) &&
    lon.toFixed(4) === lastLon.toFixed(4)
  ) {
    return;
  }
  ...
  httpGet(url, (err, response) => {
    ...
    try {
      ...
      console.error("[Kia] Geocoding Fehler: " + e);
    }
  });
}
