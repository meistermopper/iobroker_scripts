...
on({ id: ID_HOST_ALIVE, change: 'ne' }, (obj) => {
    ...
    if (!alive) {
        ...
        watchdogTimer = setTimeout(() => {
            ...
            sendTo('telegram', 'send', { text: '+++ ❌ ioBroker Zigbee ist down! +++' });
        ...
        if (watchdogTimer) {
            ...
            watchdogTimer = null;
            console.log('[Watchdog] rpizigbee ist wieder da (innerhalb des Zeitlimits).');
        }

        if (wasReportedDown) {
            // Host war bereits als "Down" gemeldet und ist nun wieder da
            sendTo('telegram', 'send', { text: '+++ ✅ ioBroker Zigbee läuft wieder. +++' });
