...
async function run() {
    ...
    if (!existsState(CONFIG.dpVIS)) await createStateAsync(CONFIG.dpVIS, '', { type: 'string', role: 'html' });
    if (!existsState(CONFIG.dpTrigger)) await createStateAsync(CONFIG.dpTrigger, false, { type: 'boolean', role: 'button' });
