...
function getSmartName(id) {
    ...
    return name.toString().replace(/:\d+.*$/g, "").replace(/\.battery$|\.percent$|\.low_bat$/i, "").trim();
}
...
function evaluateVoltage(v) {
    let res = { status: CONFIG.symbols.ok, color: CONFIG.design.colorOk, isCrit: false };
    if (v > 3.2) {
        if (v <= CONFIG.thresholds.bigBattCrit) { res.status = CONFIG.symbols.crit; res.color = CONFIG.design.colorCrit; res.isCrit = true; }
        else if (v <= CONFIG.thresholds.bigBattWarn) { res.status = CONFIG.symbols.warn; res.color = CONFIG.design.colorWarn; }
    } else if (v <= 1.5) {
        if (v < 1.1) { res.status = CONFIG.symbols.crit; res.color = CONFIG.design.colorCrit; res.isCrit = true; }
        else if (v <= 1.2) { res.status = CONFIG.symbols.warn; res.color = CONFIG.design.colorWarn; }
    } else {
        if (v < 2.2) { res.status = CONFIG.symbols.crit; res.color = CONFIG.design.colorCrit; res.isCrit = true; }
        else if (v <= 2.5) { res.status = CONFIG.symbols.warn; res.color = CONFIG.design.colorWarn; }
    }
    return res;
}
    ...
    for (const conf of ADAPTER_CONFIG) {
        ...
        states.each(id => {
            ...
            if (conf.type === 'percent') {
                if (valNum <= CONFIG.thresholds.crit) { color = CONFIG.design.colorCrit; status = CONFIG.symbols.crit; isCrit = true; }
                else if (valNum <= CONFIG.thresholds.warn) { color = CONFIG.design.colorWarn; status = CONFIG.symbols.warn; }
            } else if (conf.type === 'volt') {
                const evalV = evaluateVoltage(valNum);
                color = evalV.color; status = evalV.status; isCrit = evalV.isCrit;
            } else if (conf.type === 'bool' && val) {
                color = CONFIG.design.colorCrit; status = CONFIG.symbols.crit; isCrit = true;
            }
...
async function sendNotifications(criticalDevices) {
    ...
    if (CONFIG.useTelegram) {
        sendTo('telegram', 'send', { text: message, parse_mode: 'Markdown', disable_notification: isQuietTime });
    }
    ...
    if (CONFIG.useGotify) {
        ...
        if (tokenState && tokenState.val) {
            ...
            exec(`curl "https://mygotify.meistermopper.de/message?token=${tokenState.val}" -F "title=Batterie Alarm" -F "message=${message}" -F "priority=${prio}"`);
        }
    }
}
