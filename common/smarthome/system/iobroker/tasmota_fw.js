...
async function checkTasmotaVersion(retryCount = 0) {
    ...
    if (logging && retryCount > 0) console.log(`Tasmota: Wiederholung der Abfrage... (Versuch ${retryCount + 1}/${maxRetries})`);
    ...
    httpGet(url, { 
        ...
        if (err || !response || response.statusCode !== 200) {
            const errorMsg = err ? err.message : (response ? `Status ${response.statusCode}` : 'Keine Antwort');
            ...
            if (retryCount < maxRetries - 1) {
                ...
                console.error(`Tasmota: GitHub konnte nach ${maxRetries} Versuchen nicht erreicht werden. Abbruch.`);
            }
            return;
        }
        ...
        try {
            ...
            if (latestFullVersion !== lastKnownVersion && updateDevices.length > 0) {
                ...
                if (logging) console.warn(`Tasmota: Update-Meldung versendet für ${updateDevices.length} Geräte.`);
            } else if (logging) {
                console.log(`Tasmota: Check beendet. System ist auf dem neuesten Stand (${latestFullVersion}).`);
            }
