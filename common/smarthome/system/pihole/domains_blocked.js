...
const piholeServers = [
  ...
  {
    host: "pihole",
    datapoint: "0_userdata.0.Pihole.pihole0.UniqueGravityDomains",
  },
  {
    host: "rpifr24",
    datapoint: "0_userdata.0.Pihole.pihole1.UniqueGravityDomains",
  },
];
...
function updatePiHoleData() {
  ...
  piholeServers.forEach((server) => {
    ...
    require("node:child_process").exec(command, (error, stdout, stderr) => {
      if (error) {
