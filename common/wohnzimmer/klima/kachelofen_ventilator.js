...
on({ id: ID_TEMP_KACHELOFEN, change: 'ne' }, (obj) => {
    ...
    if (isHeizmonat && temp >= 23 && oldTemp < 23 && !getState(ID_FAN_SWITCH).val && !einschaltenAktiv) {
        turnFanOn(`Temperatur am Kachelofen: ${temp}°C.`);
    }

    // LOGIK 2: Sommer-Modus (Wohnzimmer Temp)
    if (!isHeizmonat && temp >= 25 && oldTemp < 25 && !getState(ID_FAN_SWITCH).val && !einschaltenAktiv) {
