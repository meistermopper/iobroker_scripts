...
function processFinishT() {
  ...
  const priceKwh = getState(ID_PRICE_T)?.val || 0.3;

  const diffEnergy = Math.max(0, endEnergy - startEnergyT);
  const totalCost = diffEnergy * priceKwh;

  console.log(
    `Trockner: Ende erkannt, Start ${startEnergyT.toFixed(3)} kWh -> Ende ${endEnergy.toFixed(3)} kWh = Diff ${diffEnergy.toFixed(3)} kWh`,
  );
  ...
  const msg =
    `☀️💨 Der Trockner ist fertig. Dauer: ${timeStr}. ` +
