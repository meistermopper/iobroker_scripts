...
async function ensureStatesExist() {
  const states = [
    ...
    { id: "targetTemp", type: "number", role: "level.temperature", unit: "°C", def: 90 },
    {
      ...
      def: false,
      name: "Tür-Sicherheit (geschlossen)",
    },
    {
      id: "remoteControl",
      type: "boolean",
      ...
      def: false,
      name: "Remote-Steuerung aktivierbar",
    },
    { id: "errorMsg", type: "string", role: "text", def: "" },
    {
      ...
      role: "indicator",
      def: false,
      name: "10-Minuten-Vorwarnung gesendet",
    },
    {
      ...
      role: "indicator",
      def: false,
      name: "Zieltemperatur erreicht gesendet",
    },
  ];
...
async function updateStatus() {
  ...
  try {
    ...
    try {
      const responseState = await client.get(
        `${deviceBaseUrl}/devices/state?deviceId=${FIXED_ID}`,
        {
          headers: { Authorization: `Bearer ${idToken}`, "x-harvia-partner-id": PARTNER_ID },
        },
      );
    ...
    if (p || s) {
      ...
      if (p) {
        ...
        const currentTemp = getApiValue(p, [
          "temperature",
          "temp",
          "current_temperature",
          "ambient_temperature",
        ...
        const bathingHours = getApiValue(p, [
          "totalBathingHours",
          "total_bathing_hours",
          "bathing_hours",
        ]);
        ...
        const operatingHours = getApiValue(p, [
          "totalOperatingHours",
          "totalHours",
          "total_hours",
          "operating_hours",
